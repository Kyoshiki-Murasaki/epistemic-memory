...
| MemGov-Bench | Not implemented | No benchmark scores or datasets in this release |
...
|---|---|---|
| Closed epistemic status separate from currentness | Implemented and tested | Not evaluated in M10 |
| Per-decision trust policy | Implemented and tested | Not evaluated in M10 |
| Action gate separate from retrieval | Implemented and tested | Not evaluated in M10 |
| Downstream invalidation | Implemented and tested | Not evaluated in M10 |
| Immutable historical decision explanation | Implemented and tested | Not evaluated in M10 |
| Per-agent scope/action permissions | Implemented and tested | Not evaluated in M10 |
| Proposal and ephemeral modes | Implemented and tested | Not evaluated in M10 |

No superiority claim or MemGov-Bench result is made. The narrower contribution is the tested
combination of governance properties and their placement at one service boundary.
...
python -m pytest -q
python -m compileall -q epistemic_memory examples tests
python -m pip check
git diff --check
```
...
- No MemGov-Bench implementation, dataset, score, or later-milestone work.
...
is currently declared; do not assume reuse terms that the repository has not granted. This M10
checkpoint is local release documentation only: it does not publish a package, create a hosted
release, or push a remote branch.
